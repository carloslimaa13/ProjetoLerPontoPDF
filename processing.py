...
class PontoProcessor:
    ...
    def _extract_page_info(
            ...
            if "NOME:" in linha.upper() and not nome:
                ...
                if cand_mesma_linha:
                    ...
                    for j in range(i + 1, min(i + 15, len(linhas))):
                        ...
                        if cand.lower() == "isento":
                            continue

                        if "HORÁRIO DE TRABALHO" in cand.upper():
                            continue

                        if cand.isdigit():
                            continue

                        if re.match(r"^\d{2}\.\d{3}\.\d{3}/", cand):
                            continue

                        if re.match(r"^\d{3}\.\d{3}\.\d{3}-", cand):
                            continue
        ...
        if dates_info:
            ...
            maior_grupo = max(
                grupos_x,
                key=lambda g: len(g['elementos'])
            )

            elementos_datas = maior_grupo['elementos']

            if (
                x_normais is not None
                or x_faltas is not None
                or x_extras is not None
            ):
                    ...
                    for w in words:
                        ...
                        if re.match(r"^\d{2,3}:\d{2}$", txt):
                            ...
                            if abs(y_word - y_data) < 10:
                                ...
                                if (
                                    x_normais
                                    and abs(x_word - x_normais) < 20
                                ):
                                    hora_encontrada = txt

                                elif (
                                    x_faltas
                                    and abs(x_word - x_faltas) < 20
                                ):
                                    falta_encontrada = txt

                                elif (
                                    x_extras
                                    and abs(x_word - x_extras) < 20
                                ):
                    ...
                    for page in pdf:
                        ...
                        if info:
                            ...
                            if chave not in dados_globais:
                                ...
                                dados_globais[chave]["Registros"].update(
                                    info["Registros"]
                                )
